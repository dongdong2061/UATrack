...
def get_optimizer_scheduler(net, cfg):
    ...
    if 'bat' in train_type:
        ...
        if is_main_process():
            print("All parameters are shown below.")
            total = 0
            adap_total = 0
            for n, p in net.named_parameters():
                print(f"{n}: {p.numel()} params, requires_grad={p.requires_grad}")
                total += p.numel()
                if 'adap' in n.lower():  # 忽略大小写匹配
                    adap_total += p.numel()
            print(f"Total parameters: {total}")
            print(f"Parameters containing 'adap': {adap_total}")
